...
// 重新绘制页面
  ...
  $submit.addEventListener("touchstart",function(data){
    ...
    if(this.classList.contains("xinzeng")){
      ...
      this.classList.remove("xinzeng");
    }else{
        var url1='/php/changelianxiren.php/?id='+num+'&name='+name+'&phone='+phone+'&beizhu='+beizhu;
        $submit.innerHTML="保存中..."
        $.ajax(url1,function(data){
          contacts=contacts.filter(function(v){
            return v.id!==num;
          })
          contacts.push({id:num,name:name,phone:phone,beizhu:beizhu});
          hide();
          render();
          $submit.innerHTML="保存"
        })
